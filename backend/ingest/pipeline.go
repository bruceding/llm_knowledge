...
2. 更新 {{.WikiDir}}/index.md，添加此文档条目（链接格式：[{{.Name}}](sources/{{.Name}}.md)）
...
5. 更新索引文件（如果不存在则创建）：
   - {{.WikiDir}}/sources.md — 列出所有源文档及其简介
   - {{.WikiDir}}/entities.md — 列出所有实体及其简介
   - {{.WikiDir}}/topics.md — 列出所有主题及其简介

6. 在 {{.WikiDir}}/log.md 追加操作日志
