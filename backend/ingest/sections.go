...
1. 先讲清楚这一章在做什么、为什么需要、核心思路，不限字数，讲透为止。
2. 如果本章涉及算法 / 方法 / 模型：分步拆解它的工作流程，用直觉和类比帮助理解；必要时可用表格、公式辅助说明，但不要照抄原文公式而不加解释。
3. 如果本章点明了某个待解决的问题或挑战：说明它是什么、现有方法为什么不够、作者如何应对。
4. 可以使用你自己的知识补充背景信息，帮助读者建立更完整的理解。
5. 最后以 "## 关键要点" 为标题，列 3-5 条一句话要点；不适用项可省略。
`
