...
这是一篇学术论文的一个章节，章节标题为「%s」。请用中文深入讲解这一章节，目标是让读者不读原文也能完全理解所有技术细节。

输出要求（纯 Markdown，不要输出任何额外说明）：
1. 先讲清楚这一章在做什么、为什么需要、核心思路。
2. 如果本章涉及算法 / 方法 / 模型：
   - 分步拆解完整的工作流程，包括输入、每一步的操作、输出。
   - 用伪代码块或分步编号展示算法的具体步骤，不要只说"用了某某算法"就跳过。
   - 用直觉类比帮助理解（例如用表格展示每一层/每一步在做什么）。
   - 必要时可用公式辅助，但每个公式都要附带通俗解释。
3. 如果本章点明了某个待解决的问题或挑战：说明它是什么、现有方法为什么不够、作者如何应对。
4. 补充背景知识：用你自己的知识解释关键概念的来龙去脉，帮助读者建立更完整的理解。
5. 说明经过本章处理后的最终效果：输入变成了什么、输出变成了什么、对后续章节有什么意义。
6. 最后以 "## 关键要点" 为标题，列 3-5 条一句话要点；不适用项可省略。
